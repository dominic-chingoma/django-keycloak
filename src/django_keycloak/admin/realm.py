...
from django_keycloak.models import Client, OpenIdConnectProfile, RemoteClient
...
refresh_open_id_connect_well_known.short_description = (
...
def refresh_certs(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request=request, message="Certificates refreshed", level=messages.SUCCESS
    )
...
def clear_client_tokens(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request=request, message="Tokens cleared", level=messages.SUCCESS
    )
        ...
        try:
            ...
            if e.response.status_code == 403:
                modeladmin.message_user(
                    ...
                    'account has the "keycloak_client" role?'.format(realm.name),
                ...
                raise
    modeladmin.message_user(
        request=request, message="Permissions synchronized", level=messages.SUCCESS
    )
        ...
        try:
            ...
            if e.original_exc.response.status_code == 400:
                modeladmin.message_user(
                    ...
                    'Management"  enabled for the related client?'.format(realm.name),
                    level=messages.ERROR,
                )
                return
            else:
                raise
    modeladmin.message_user(
        request=request, message="Resources synchronized", level=messages.SUCCESS
    )


synchronize_resources.short_description = "Synchronize models as Keycloak " "resources"
